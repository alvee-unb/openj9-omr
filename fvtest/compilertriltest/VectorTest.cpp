...
double outputDouble[] =  {0.0, 0.0};
double inputADouble[] =  {1.0, 2.0};
double inputBDouble[] =  {1.0, 2.0};

class VDoubleArithmeticTest : public TRTest::JitTest,
                 public ::testing::WithParamInterface<std::string> {};

TEST_P(VDoubleArithmeticTest, UsingDouble) {
    auto param = GetParam();

    std::string arch = omrsysinfo_get_CPU_architecture();
    SKIP_IF(("vsub" == param || "vmul" == param) && (OMRPORT_ARCH_X86 == arch || OMRPORT_ARCH_HAMMER == arch), MissingImplementation);
    ...
    char trilCode[1024] = {0};
    std::snprintf(trilCode,
                    sizeof(trilCode),
                    trilTemplate,
                    param.c_str());

    auto ast = parseString(trilCode);

    ASSERT_NOTNULL(ast) << "Parsing failed unexpectedly.\n";

    Tril::DefaultCompiler compiler(ast);

    ASSERT_EQ(0, compiler.compile()) << "Compilation failed unexpectedly\n" << "Input trees: " << trilCode;

    auto entry_point = compiler.getEntryPoint<void (*)(double[],double[],double[])>();

    entry_point(outputDouble,inputADouble,inputBDouble);

    if ("vadd" == param) {
        for (int i = 0; i < (sizeof(outputDouble) / sizeof(*outputDouble)); i++) {
            EXPECT_EQ(inputADouble[i] + inputBDouble[i], outputDouble[i]);
        }
    } else if ("vsub" == param) {
        for (int i = 0; i < (sizeof(outputDouble) / sizeof(*outputDouble)); i++) {
            EXPECT_EQ(inputADouble[i] - inputBDouble[i], outputDouble[i]);
...
INSTANTIATE_TEST_CASE_P(VectorTest, VDoubleArithmeticTest,
    ::testing::Values("vadd", "vsub", "vmul", "vdiv"));
